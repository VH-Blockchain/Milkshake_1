...
import { calculateCakeEarnedPerThousandDollars, apyModalRoi } from "../farm/compoundApy";
...
const FarmTab = () => {
  ...
  useEffect(() => {
    ...
    apr(lpTokens);
    totalData(lpTokens, userlpTokens);
  ...
  const totalData = (lpTokens, userlpTokens) => {
    var a = [];

    if (lpTokens && userlpTokens) {
      for (let i = 0; i < 7; i++) {
        let employee = {
          ...lpTokens[i],
          ...userlpTokens[i],
  ...
  const apr = async (lpTokens) => {
    ...
    if (lpTokens) {
      lpTokens.map((farm, i) => {
        let cakePrice = new BigNumber(0.3516);
        ...
        let apy = cakePrice2.times(cakeRewardPerYear); //0.347 * 245,034.72 = 85,027.04784
        ...
        if (farm.quoteTokenSymbol === "BNB") {
          ...
          totalValue = totalValue.times(ethPrice2);
  ...
  // console.log(a,"ggggggggggggggggggggg");
